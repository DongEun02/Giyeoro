...
- 공개 GitHub 활동·사용 언어·프로젝트 의존성 기반 맞춤 저장소 추천
...
GitHub OAuth App의 callback URL을 `${APP_URL}/api/auth/github/callback`으로 등록한 뒤 다음 환경 변수를 설정합니다. 로그인은 공개 프로필을 확인하는 `read:user` 권한만 사용하며 비공개 저장소 권한은 요청하지 않습니다.
...
배포 환경에서는 `APP_URL`을 실제 서비스 주소로 바꾸고 네 변수를 Vercel 프로젝트 설정에도 등록합니다. 로컬과 배포 환경에서 로그인을 모두 시험하려면 callback URL이 서로 다르므로 OAuth App을 환경별로 나누는 편이 안전합니다.

맞춤 프로젝트 추천은 로그인 사용자의 공개 저장소, 최근 30일 공개 이벤트, 공개 저장소 루트의 `package.json`, `pom.xml`, `build.gradle`, `pyproject.toml` 등만 분석합니다. 비공개 저장소 권한은 요청하지 않으며, 추천 후보는 최근 개발 활동·라이선스·기여 가이드·입문 라벨 이슈·외부 Pull Request 응답을 다시 확인합니다.
