...
let latestTrendingEntries: { value: any[]; cachedAt: number } | null = null;
let trendingEntriesInFlight: Promise<any[]> | null = null;
...
const plainTextFromHtml = (value: any) => String(value || "")
  .replace(/<[^>]+>/g, " ")
  .replace(/&nbsp;/g, " ")
  .replace(/&amp;/g, "&")
  .replace(/&#39;/g, "'")
...
export const parseMonthlyTrendingHtml = (html: any) => {
...
const mapWithConcurrency = async (items: any, concurrency: any, mapper: any) => {
...
const readGithubError = (response: any) => {
...
const fetchRepositoryMetadata = async (trendingRepository: any, githubToken: any) => {
...
const attachContributionGuide = async (repository: any, githubToken: any) => {
...
const loadMonthlyTrendingRepositories = async (githubToken: any) => {
  const parsedRepositories = await getMonthlyTrendingEntries();

  const metadataResults = await mapWithConcurrency(
    parsedRepositories,
    6,
    (repository: any) => fetchRepositoryMetadata(repository, githubToken)
  );
  ...
  const guideResults = await mapWithConcurrency(
    ...
    6,
    (repository: any) => attachContributionGuide(repository, githubToken)
  );
...
export const getMonthlyTrendingRepositories = async (
  { githubToken, force = false }: { githubToken?: string; force?: boolean } = {}
) => {
...
const toPublicRepository = (repository: any) => {
...
export const handleTrendingRepositoriesRequest = async (request: any, response: any, options: HandlerOptions = {}) => {
  const {
